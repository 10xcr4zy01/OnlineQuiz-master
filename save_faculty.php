<?php 
	...
	if(empty($id)){
		...
		if($insert_user){
			$id = $conn->insert_id;
			$insert_faculty =$conn->query("INSERT INTO lecturer set user_id = '".$id."', subject='".$subject."' ");
			if($insert_faculty){
		...
		if($update_user){
			$update_faculty =$conn->query("UPDATE lecturer set subject='".$subject."' where id = '".$id."' ");
			if($update_faculty){
				echo json_encode(array('status'=>1));
			}
		}
